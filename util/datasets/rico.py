...
from util.constraint import xywh_to_ltrb_split
...
class _RicoDataset(BaseDataset):
    ...
    def process(self):
        ...
        with fs.open(raw_file, "rb") as f, ZipFile(f) as z:
            ...
            for name in tqdm(names, total=len(names), ncols=150, desc='prepare Rico'):
        ...
        with fs.open(self.processed_paths[0], "wb") as file_obj:
            ...
            torch.save(self.collate(data_list[: s[0]] * 10), file_obj)
    ...
    def get_original_resource(self, batch) -> Image:
        ...
        for i, name in enumerate(names):
            ...
            W, H = img.size
            ltrb = xywh_to_ltrb_split(bbox[i].T.numpy())
