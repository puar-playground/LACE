...
from .data_util import RelLoc, RelSize, detect_loc_relation, detect_size_relation
from .constraint import xywh_to_ltrb_split
...
def compute_alignment(bbox: FloatTensor, mask: BoolTensor) -> FloatTensor:
    """
...
def compute_overlap(bbox: FloatTensor, mask: BoolTensor):
    """
    Based on
    ...
    l1, t1, r1, b1 = xywh_to_ltrb_split(bbox.unsqueeze(-1))
    l2, t2, r2, b2 = xywh_to_ltrb_split(bbox.unsqueeze(-2))
    a1 = (r1 - l1) * (b1 - t1)
    ...
    return results["overlap-LayoutGAN++"]
...
def compute_iou(
    ...
    l1, t1, r1, b1 = xywh_to_ltrb_split(box_1.T)
    l2, t2, r2, b2 = xywh_to_ltrb_split(box_2.T)
...
def compute_perceptual_iou(
    ...
    l1, t1, r1, b1 = [
        ...
        for x in xywh_to_ltrb_split(unique_box_1.T)
    ]
    canvas = np.zeros((N, N))
